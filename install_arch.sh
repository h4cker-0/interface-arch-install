...
function install_arch(){
	...
	echo -e "\e[32m[*]\e[0m\e[32\n\nInstalando paru \e[0m"
        mkdir ~/repos && cd ~/repos/ && git clone https://aur.archlinux.org/paru.git && cd paru && makepkg -si && yay -S paru && clear
	...
	sudo pacman -S p7zip wget xclip 
	...
	mkdir ~/.config/kitty/ && cd ~/.config/kitty/ && mv ~/repos/interface-arch-install/kitty.conf . && mv ~/repos/interface-arch-install/color.ini . && sudo cp ~/repos/interface-arch-install/rc.lua ~/.config/awesome/ && sudo cp ~/repos/interface-arch-install/init.lua ~/.config/awesome/ui/decorations/init.lua
	echo -e "\e[32m[*]\e[0m\e[33mInstalando algunas herramientas de pentesting\e[0m"
	paru -S nmap burpuite python-pip python3 python2 wfuzz apache
